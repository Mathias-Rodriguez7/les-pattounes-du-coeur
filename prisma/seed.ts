...
async function main() {
    ...
    for (let i = 27; i < 107; i++) { // 27 à 106 (80 hosts)
    ...
    for (const cat of cats.slice(0, 50)) { // ~33% des chats
        if (randomBool(0.6)) {
    ...
    for (const cat of cats) {
        // Chaque chat a 1-3 volunteers référents
        const randomVols = faker.helpers.arrayElements(volunteers, faker.number.int({ min: 1, max: 3 }));
    ...
    for (const cat of cats.slice(0, 80)) { // ~53% des chats ont un placement
    ...
    for (const cat of cats.slice(0, 100)) { // ~67% des chats ont des soins
    ...
    for (const cat of cats.slice(0, 30)) { // ~20% des chats sont adoptés
